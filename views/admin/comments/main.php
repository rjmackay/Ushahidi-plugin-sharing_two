						...
						<table class="table">
							...
							<tbody>
								...
								foreach ($comments as $comment)
								{
									...
									$sharing_incident_comment = FALSE;
									// If no incident id, check for sharing incident comment
									if ( ! $incident_id )
									{
										$sharing_incident_comment = ORM::factory('sharing_incident_comment')->with('sharing_incident')->where('comment_id', $comment_id)->find();
										$incident_id = $sharing_incident_comment->sharing_incident->id;
										$incident_title = $sharing_incident_comment->sharing_incident->incident_title;
									}
									?>
									<tr>
										...
										<td class="col-2">
											<div class="post">
												...
												<?php
													if ($incident_title != "" )
													{
														?><div class="comment_incident"><?php echo Kohana::lang('ui_main.in_response_to');?>: <strong><a href="<?php echo url::base() . 'reports/sharing/view/' . $incident_id; ?>"><?php echo strip_tags($incident_title); ?></a></strong></div><?php
													}
												}
												else
												{
